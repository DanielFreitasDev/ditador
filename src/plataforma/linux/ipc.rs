...
/// O ouvinte, no vocabulário do `crate::ipc`.
pub type Escuta = UnixListener;

/// O que o `bind` devolve quando não conseguiu ficar com o socket.
pub enum Falha {
    JaRodando,
    SemLugar(String),
}
...
///
/// É o mesmo raciocínio da DACL do named pipe no Windows, escrito com as
/// ferramentas de cá: o canal de controle é do usuário e de mais ninguém.
...
pub fn bind() -> Result<Escuta, Falha> {
    let Some(path) = socket_path() else {
        return Err(Falha::SemLugar(
            "sem um lugar seguro para o socket".to_string(),
        ));
    };

    if UnixStream::connect(path).is_ok() {
        return Err(Falha::JaRodando);
    }
    ...
    match UnixListener::bind(path) {
        ...
        //
        // O Windows não precisa desta segunda conferência —
        // `FILE_FLAG_FIRST_PIPE_INSTANCE` faz o conferir e o criar serem a mesma
        // chamada, e a corrida não existe.
        Err(e) => {
            if UnixStream::connect(path).is_ok() {
                Err(Falha::JaRodando)
            } else {
                Err(Falha::SemLugar(format!("criando {}: {e}", path.display())))
            }
        }
    }
}
